...
module Sord
  ...
  class RbiGenerator    
    ...
    # signatures to the current RBI file. This does not add attributes.
    # @param [YARD::CodeObjects::NamespaceObject] item
    # @return [void]
    def add_methods(item)
      item.meths(inherited: false).each do |meth|
        ...
        # If the method is an instance attribute, skip it. It will have its
        # attr_whatever generated by #add_attributes.
        if meth.is_attribute? && meth.scope == :instance
          next
        end
    ...
    # Given a YARD NamespaceObject, add lines for its instance attributes to the  
    # RBI file. Only instance attributes are currently supported, because
    # Parlour does not have a stable way of generating "class << self" sections.
    # @param [YARD::CodeObjects::NamespaceObject] item
    # @return [void]
    def add_attributes(item)
      item.attributes[:instance].each do |name, info|
        ...
        returns = if return_tags.length == 0
          Logging.omit("no YARD attribute type given, using T.untyped", read || write)
          type = 'T.untyped'
        elsif return_tags.length == 1 && return_tags&.first&.types&.first&.downcase == "void"
          type = nil
        else
